...
function execute(url, page) {
    if (!page) page = '1';
    var host = 'https://mangakakalot.com';
    var doc = Http.get(url).params({"page": page}).html();

    var next = $.Q(doc, 'div.panel_page_number > div.group_page > a.page_select + a').text();

    var elems = $.QA(doc, 'div.list-truyen-item-wrap');
    var data = [];
    elems.forEach(function(e) {
        data.push({
            ...
            description: $.Q(e, 'a.list-story-item-wrap-chapter').text(),
            host: host
        })
    });
